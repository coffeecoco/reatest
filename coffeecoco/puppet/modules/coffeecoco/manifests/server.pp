class coffeecoco::server {
  ...
  $server = $coffeecoco::params::hiera['server']
  ...
  exec { 'dotfiles':
    cwd     => $user_home,
    command => "cp -r /vagrant/coffeecoco/files/dot/.[a-zA-Z0-9]* ${user_home}/ && \
                chown -R ${::ssh_username} ${user_home}/.[a-zA-Z0-9]* && \
                cp -r /vagrant/coffeecoco/files/dot/.[a-zA-Z0-9]* /root/",
    onlyif  => 'test -d /vagrant/coffeecoco/files/dot',
    returns => [0, 1],
  ...
  case $::osfamily {
    ...
    default: {
      error('coffeecoco currently only works with Debian and RHEL families')
    }
  }
  ...
  case $::operatingsystem {
    ...
    default: {
      error('coffeecoco supports Ubuntu, CentOS and RHEL only')
    }
  }
