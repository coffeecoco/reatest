...
if [[ ! -d '/.coffeecoco-stuff' ]]; then
    mkdir '/.coffeecoco-stuff'
fi

touch '/.coffeecoco-stuff/vagrant-core-folder.txt'
echo "${VAGRANT_CORE_FOLDER}" > '/.coffeecoco-stuff/vagrant-core-folder.txt'

if [[ -f '/.coffeecoco-stuff/initial-setup' ]]; then
    exit 0
fi

if [[ "${OS}" == 'debian' || "${OS}" == 'ubuntu' ]]; then
    wget --quiet --tries=5 --connect-timeout=10 -O /.coffeecoco-stuff/puppetlabs.gpg \
        https://apt.puppetlabs.com/pubkey.gpg
    apt-key add /.coffeecoco-stuff/puppetlabs.gpg
...
if [[ ! -f '/.coffeecoco-stuff/disable-tty' ]]; then
    perl -pi'~' -e 's@Defaults(\s+)requiretty@Defaults !requiretty@g' /etc/sudoers

    touch '/.coffeecoco-stuff/disable-tty'
fi

touch '/.coffeecoco-stuff/initial-setup'
