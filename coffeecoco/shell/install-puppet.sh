...
if [[ ! -f '/.coffeecoco-stuff/install-puppet' ]]; then
    if [[ "${OS}" == 'debian' || "${OS}" == 'ubuntu' ]]; then
        URL="https://apt.puppetlabs.com/puppetlabs-release-pc1-${CODENAME}.deb"
        wget --quiet --tries=5 --connect-timeout=10 -O /.coffeecoco-stuff/puppetlabs-release-pc1.deb ${URL}
        dpkg -i /.coffeecoco-stuff/puppetlabs-release-pc1.deb
        apt-get update
    ...
    touch /.coffeecoco-stuff/install-puppet
fi
