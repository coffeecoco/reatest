...
function create_key()
{
    ...
    if [[ ! -f "${VAGRANT_CORE_FOLDER}/files/dot/ssh/${BASE_KEY_NAME}" ]]; then
        ...
        echo "Your private key for SSH-based authentication has been saved to 'coffeecoco/files/dot/ssh/${BASE_KEY_NAME}'!"
    else
        echo "Pre-existing private key found at 'coffeecoco/files/dot/ssh/${BASE_KEY_NAME}'"
    fi
}
